...
angular.module('permitCalc', ['ngTouch']).factory('permitFactory', function($http, $q, $filter){
	...
	return {
		...
		template: '<table><tbody><tr><td>Building Type</td><td><select ng-options="group as group.group for group in groups" ng-model="selectedGroup"></select></td></tr><tr><td>Construction Type</td><td><select ng-options="param as param for param in params" ng-model="selectedParam"></select></td></tr><tr><td>Scope of Construction</td><td><select ng-options="constructionScope as constructionScope.name for constructionScope in constructionScopes" ng-model="selectedconstructionScope"></select></td><tr><td>Square Feet</td><td><input type="number" ng-model="squareFeet"/></td></tr><tr><td>Construction Valuation</td><td>{{calculateValuation() | currency : $ : 0}}</td><tr><td>Building Permit</td><td>{{calculateBuildingPermit() | currency : $ : 0}}</td></tr><tr><td>Plan Review Fee</td><td>{{calculatePlanReviewFee(percents.planReview.all) | currency : $ : 0}}</td></tr><tr><td>Electrical Permit</td><td>{{calculateElectricalPermit(percents.electricalPermit.all) | currency : $ : 0}}</td></tr><tr><td>Plumbing Permit</td><td>{{calculatePlumbingPermit(percents.plumbingPermit.all) | currency : $ : 0}}</td></tr><tr><td>Mechanical Permit</td><td>{{calculateMechanicalPermit(percents.mechanicalPermit.all) | currency : $ : 0}}</td></tr><tr><td><strong>Total Permit Price</strong></td><td><strong>{{calculateTotalPrice() | currency : $ : 0}}</strong></td></tr><tr><td>% of Valuation</td><td>{{(totalPrice/valuation) * 100 | number: 2}}%</td></tr></tbody></table>',
